...
def sub(s):
    tr = dict(zip("aeox0123456789+-=()", "ₐₑₒₓ₀₁₂₃₄₅₆₇₈₉₊₋₌₍₎"))
    try:
        return ''.join(tr[c] for c in s)
    except (KeyError, TypeError):
        return ''
...
def metadata(p, doc):
    ...
    return Source(
        'article', 'glossa{}'.format(p.stem),
        author=names(doc.xpath(".//contrib[@contrib-type='author']/name")),
        title=title,
        year=doc.xpath(".//pub-date/year")[0].text,
        volume=doc.xpath(".//volume")[0].text,
        issue=doc.xpath(".//issue")[0].text,
        doi=doi,
